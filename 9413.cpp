...
int main() {
	...
	while (t--) {
		...
		MCMF mf(n * 2 + 3, 0, n * 2 + 2);
		for (int i = 1; i <= n; i++) mf.add_edge(i, i + n, 1, -1);
		mf.add_edge_from_source(n * 2 + 1, 2, 0);
		for (int i = 1; i <= n; i++) {
			mf.add_edge(n * 2 + 1, i, inf, 0);
			mf.add_edge_to_sink(i + n, inf, 0);
		}
		for (int i = 0; i < m; i++) {
			int x, y; scanf(" %d %d", &x, &y);
			mf.add_edge(x + n, y, inf, 0);
		}
