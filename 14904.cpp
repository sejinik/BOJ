...
int main() {
	scanf(" %d %d", &n, &m);
	MCMF mf(n*n * 2 + 3, n*n * 2 + 1, n*n * 2 + 2);
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			scanf(" %d", &arr[i][j]);
			mf.add_edge(i*n + j, i*n + j + n*n, 1,-arr[i][j]);
		}
	}
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			for (int k = 0; k < 2; k++) {
				...
				if (0 <= nx && nx < n && 0 <= ny && ny < n) {
					mf.add_edge(i*n + j + n*n, nx*n + ny, m, 0);
					mf.add_edge(i*n + j, nx*n + ny, m, 0);
				}
			}
		}
	}
	mf.add_edge_from_source(0, m, 0);
	mf.add_edge_to_sink(n*n - 1, m, 0);
	mf.add_edge_to_sink(n*n - 1 + n*n, m, 0);
